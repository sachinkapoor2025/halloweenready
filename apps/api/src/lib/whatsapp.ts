...
export function abandonedCouponWhatsAppMessage(input: {
  ...
  return `Hi! This is ${SITE}. Complete your Halloween order with ${input.discountPercent}% off using code ${input.code}. Valid until ${expiry} (ET). Checkout: ${url}`;
}
...
export function abandonedCartWhatsAppMessage(input: {
  ...
  const nudge =
    input.reminder === 1
      ? "You left Halloween items in your cart."
...
Complete payment so we can pack and ship before Halloween.`;
}
