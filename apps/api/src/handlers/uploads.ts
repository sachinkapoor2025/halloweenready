...
import { IMAGE_CACHE_CONTROL, allVariantObjectKeys } from "@halloweenready/shared";
...
async function recordUploadRegistry(slug: string, imageUrl: string, storageKey: string | null) {
  ...
  const { uploadRegistryKeys } = await import("@halloweenready/shared");
...
export async function attachImageToProduct(event: APIGatewayProxyEventV2) {
  ...
  const { productKeys, mergeProductImages } = await import("@halloweenready/shared");
