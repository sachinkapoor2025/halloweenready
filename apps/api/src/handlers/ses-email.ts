...
export async function updateSettings(event: APIGatewayProxyEventV2) {
  ...
  if (/^(smtp|mail)\.halloweenready\.com$/.test(host)) {
    return badRequest(
      "Marketing SMTP must use Mailercloud (smtp-prod.mailrcld.com). smtp.halloweenready.com is reserved for transactional order emails only."
    );
  }
  ...
  const settingsToSave = {
    ...
    smtpUser: parsed.data.smtpUser?.trim() || "order@halloweenready.com",
...
export async function sendTest(event: APIGatewayProxyEventV2) {
  ...
  try {
    ...
    const settings = await loadSettings();
    const fromName = (campaign.senderName || settings.defaultSenderName || "HalloweenReady").trim();
    // From must be Mailercloud verified Sender ID (email@halloweenready.com), not SMTP login.
    const fromEmail = (
    ...
    if (!fromEmail) {
      return badRequest(
        "Sender email is missing. Set Default sender email to your verified Mailercloud Sender ID (e.g. email@halloweenready.com)."
      );
    }
...
async function sendQueuedEmail(
  ...
  await sendViaSes({
    ...
    html,
    text: htmlToText(html),
    fromName: (campaign.senderName || settings.defaultSenderName || "HalloweenReady").trim(),
    fromEmail: (
      campaign.senderEmail ||
      settings.defaultSenderEmail ||
      "email@halloweenready.com"
