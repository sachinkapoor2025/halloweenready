...
export default function AdminWelcomeLeadsPage() {
  ...
  const exportCsv = () => {
    downloadCsv("halloween-discount-of-the-day-leads.csv", [
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">
            Halloween spin-the-wheel signups (5–20% off, 1-hour validity, one spin per email per day).
          </p>
        </div>
        <button
          type="button"
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-slate-500">
                  No Discount of the Day signups yet.
                </td>
              </tr>
            ) : (
              paged.items.map((r) => {
                ...
                return (
                  <tr key={`${r.code}-${r.createdAt}`} className="border-t border-slate-100">
                    <td className="px-4 py-3">
                      {r.email ? (
                        <a href={`mailto:${r.email}`} className="text-nav hover:underline">
                          {r.email}
                        </a>
                      ) : (
                        "—"
