...
exports.referrerHostname = referrerHostname;
exports.isInternalHost = isInternalHost;
exports.extractCampaignParams = extractCampaignParams;
exports.resolveTrafficSource = resolveTrafficSource;
exports.formatTrafficTouchLabel = formatTrafficTouchLabel;
exports.isAcquisitionTouch = isAcquisitionTouch;
exports.applyTouchToJourney = applyTouchToJourney;
exports.touchKey = touchKey;
...
function extractCampaignParams(pageUrl) {
    const utm = {};
    const clickIds = {};
    let path = pageUrl ?? "";
...
function channelFor(source, medium, clickIds) {
    if (clickIds.gclid || clickIds.wbraid || clickIds.gbraid)
        return "paid_search";
    if (clickIds.msclkid)
        return "paid_search";
    if (clickIds.fbclid || clickIds.ttclid)
        return "paid_social";
    if (paidMedium(medium) && isSocialSource(source))
        return "paid_social";
    ...
    if (SEARCH_ENGINES.some((s) => s.source === source) || medium === "organic")
        return "organic_search";
    if (medium === "email" || source === "email")
        return "email";
    if (source === "direct")
        return "direct";
    if (source === "unknown" || source === "unattributed")
        return "unknown";
    if (medium === "referral")
        return "referral";
    return "referral";
...
function resolveTrafficSource(input) {
    ...
    if (!input.referrer) {
        ...
        return touch({
            ...
            channel: "direct",
            confidence: "low",
            confidenceReason: "No referrer or campaign data — labeled Direct with low confidence (may be privacy-restricted)",
        });
    }
    return touch({
        ...base,
...
function formatTrafficTouchLabel(t) {
    const parts = [t.source];
    if (t.medium && t.medium !== "none")
        parts.push(t.medium);
    if (t.campaign)
        parts.push(t.campaign);
    return parts.join(" / ");
}
function isAcquisitionTouch(t) {
...
function applyTouchToJourney(existing, next) {
    ...
    if (existing.last && existing.last.source !== next.source) {
        ...
        const already = assisted.some((a) => `${a.source}|${a.medium}|${a.campaign ?? ""}` === key);
...
function assistedFromTouches(touches) {
    if (touches.length <= 2)
        return [];
...
function overallAttributionConfidence(first, last) {
