...
exports.CATEGORY_SLUGS = void 0;
exports.classifyChatIntent = classifyChatIntent;
exports.mergeShoppingState = mergeShoppingState;
exports.buildSearchQuery = buildSearchQuery;
exports.viewAllHref = viewAllHref;
exports.missingShoppingSlots = missingShoppingSlots;
exports.slotQuickActions = slotQuickActions;
exports.scoreProductForState = scoreProductForState;
exports.toAssistantProduct = toAssistantProduct;
exports.welcomeQuickActions = welcomeQuickActions;
exports.invitationQuickActions = invitationQuickActions;
exports.productPageQuickActions = productPageQuickActions;
...
const CATEGORY_KEYWORDS = [
    ...
    { slug: exports.CATEGORY_SLUGS.decorations, label: "decorations", pattern: /\b(decor|decoration|inflatable|yard|porch|tombstone|pumpkin|lighting|lights|outdoor|indoor|haunted house)\b/i },
    { slug: exports.CATEGORY_SLUGS.party, label: "party supplies", pattern: /\b(party|balloon|banner|tableware|cups|plates|favors|games)\b/i },
...
function parseBudget(text) {
    ...
    if (under)
        return { budgetMax: Number(under[1]) };
    ...
    if (range)
        return { budgetMin: Number(range[1]), budgetMax: Number(range[2]) };
    return {};
}
...
function classifyChatIntent(text) {
    ...
    if (/start over|new search|start again|reset/.test(q))
        return "start_over";
    if (/surprise me|dealer.?s choice|show me anything/.test(q))
        return "surprise";
    if (/compar|which (one|is better)|vs\b/.test(q))
        return "product_comparison";
    if (/where is my order|track|order status|cancel (my )?order/.test(q))
        return "order_query";
    if (/return policy|refund|exchange/.test(q))
        return "return_query";
    if (/razorpay|stripe|pay|upi|card/.test(q) && !/costume|decor/.test(q))
    ...
    if (/when is halloween|why (do people )?celebrat|tradition|history of halloween|question about halloween/.test(q))
        return "general_halloween";
    if (/hosting a .{0,20}party|plan(ning)? a (halloween )?party|party for \d/.test(q))
        return "party_planner";
    if (/size|small|medium|large|\bxl\b|\bsm\b/.test(q) && /costume|fit|wear/.test(q))
        return "size_query";
    if (/help|support|whatsapp|contact|human/.test(q) && !/find|buy|costume|decor/.test(q))
        return "support";
    if (/decorat(e|ing|ion)|costume|buy|need|want|show me|looking for|find/.test(q))
        return "product_search";
    if (/category|browse|all (halloween )?(decor|costume|party)/.test(q))
        return "category_search";
    if (/idea|what should i wear|how (do i|to) decorat/.test(q))
        return "halloween_ideas";
    if (/hello|hi\b|hey|thanks/.test(q) && q.length < 24)
        return "smalltalk";
    return "product_search";
}
function mergeShoppingState(prev, text) {
    const next = { ...prev };
    ...
    if (budget.budgetMax != null)
        next.budgetMax = budget.budgetMax;
    if (budget.budgetMin != null)
        next.budgetMin = budget.budgetMin;
    if (/\b(cheaper|less expensive|lower budget|under budget)\b/i.test(text)) {
        if (next.budgetMax != null)
            next.budgetMax = Math.max(8, Math.round(next.budgetMax * 0.7));
        else
            next.budgetMax = 30;
    }
    ...
    if (/\boutdoor|yard|porch\b/i.test(text))
        next.indoorOutdoor = "outdoor";
    else if (/\bindoor|inside|table\b/i.test(text))
        next.indoorOutdoor = "indoor";
    ...
    if (loc) {
        ...
        if (loc.city)
            next.city = loc.city;
        if (loc.region)
            next.region = loc.region;
    }
    if (/\buk\b|united kingdom|britain/i.test(text))
        next.country = "GB";
    if (/\busa\b|united states|\bus\b/i.test(text))
        next.country = "US";
...
function viewAllHref(state) {
    if (state.categorySlug && !state.theme && !state.query)
        return `/categories/${state.categorySlug}`;
...
function missingShoppingSlots(state, intent) {
    if (intent === "surprise" || intent === "general_halloween" || intent === "support")
        return [];
    if (intent === "shipping_query" || intent === "order_query" || intent === "return_query")
        return [];
    if (intent === "party_planner" && !state.partySize)
        return ["partySize"];
    const missing = [];
    const isCostume = state.categorySlug === exports.CATEGORY_SLUGS.costumes || /\bcostume\b/i.test(state.query ?? "") || Boolean(state.theme);
    const isDecor = state.categorySlug === exports.CATEGORY_SLUGS.decorations || state.indoorOutdoor === "outdoor";
    if (!state.categorySlug && !state.theme && !state.query)
        missing.push("category");
    if (isCostume && !state.audience)
        missing.push("audience");
    if ((isCostume || isDecor) && !state.style && !state.theme)
        missing.push("style");
    return missing;
}
function slotQuickActions(slot) {
    if (slot === "category") {
        return [
...
function scoreProductForState(product, state, country) {
    const hay = `${product.name} ${product.tags?.join(" ") ?? ""} ${product.description ?? ""}`.toLowerCase();
    const inferredTheme = (0, homepage_ranking_1.inferProductTheme)(product.name, product.tags);
    if (state.theme) {
        const theme = state.theme.toLowerCase();
        if (!hay.includes(theme) && inferredTheme !== state.theme)
            return 0;
    }
    let score = 1;
    if (state.theme && hay.includes(state.theme.toLowerCase()))
        score += 8;
    if (state.style && hay.includes(state.style.toLowerCase()))
        score += 4;
    if (state.audience) {
        const aud = (0, homepage_ranking_1.inferProductAudience)(product.name, product.tags);
        if (aud === state.audience || (state.audience === "kids" && aud === "kids"))
            score += 5;
        else if (aud !== "all")
            score -= 2;
    }
    if (state.categorySlug && product.categorySlug === state.categorySlug)
        score += 3;
    if (state.indoorOutdoor === "outdoor" && /outdoor|yard|inflatable|porch/.test(hay))
        score += 4;
    if (state.query) {
    ...
    if (state.budgetMax != null && product.price <= state.budgetMax)
        score += 3;
    if (state.budgetMax != null && product.price > state.budgetMax)
        score -= 6;
    if (country) {
        const avail = (0, shipping_availability_1.isProductAvailableForCountry)(product, country);
    ...
    if ((product.unitsSold ?? 0) > 20)
        score += 1;
    if (state.theme && inferredTheme === state.theme)
        score += 2;
    return score;
}
...
function productPageQuickActions(name) {
    return [
