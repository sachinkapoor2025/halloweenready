...
exports.addonsForHamper = addonsForHamper;
exports.hamperCustomizationSignature = hamperCustomizationSignature;
exports.emptyHamperCustomization = emptyHamperCustomization;
exports.resolvedHamperContentSlugs = resolvedHamperContentSlugs;
exports.cartLinesMatch = cartLinesMatch;
exports.resolveHamperCustomization = resolveHamperCustomization;
exports.buildHalloweenHamperCatalogProducts = buildHalloweenHamperCatalogProducts;
exports.getHalloweenHamperDef = getHalloweenHamperDef;
...
const PRINCESS = "https://cf.cjdropshipping.com/ac2e33c9-442f-409c-971e-a48d082f1eda.png";
const BANNER = "https://www.halloweenready.com/banners/bannerpage1.png";
const YARD = "https://www.halloweenready.com/banners/bannerpage2.png";
function line(slug, name, price, image) {
    return { slug, name, price, image };
}
/** Shared swap/extra pool — family-safe fillers already on the live catalog. */
exports.HAMPER_ADDON_POOL = [
...
exports.HALLOWEEN_HAMPER_DEFS = [
    ...
    hamper("trick-or-treat-night-hamper", "Trick-or-Treat Night Hamper", 79, "Kids cloak, princess extras, lantern, pail, and glow toys.", "Everything for trick-or-treat night: cloak, princess dress and accessories, lantern, glow bucket, balloons, and activity extras. Swap included pieces at the same $79 price.", [
    ...
    hamper("apartment-haunt-hamper", "Apartment Haunt Hamper", 99, "Indoor décor, carving kit, lamp, and renter-friendly textiles.", "A complete indoor haunt for apartments: carving kit, ghost rug, RGB pumpkin lamp, blanket, wreath, and candles. Customize with add-ons — hamper price stays $99.", [
    ...
    hamper("party-in-a-box-hamper", "Party in a Box Hamper", 129, "Balloons, backdrop, inflatable, hanging ghost, and table extras.", "Host 8–12 people without hunting SKUs: inflatable, hanging mummy, wreath, balloon kits, banners, and table décor. Swap pieces at the same $129 price.", [
    ...
    hamper("witch-night-hamper", "Witch Night Hamper", 149, "Adult witch dress, socks, wreath, lamp, and spell-night extras.", "The #1 adult Halloween look in one box: costume dress, over-knee socks, wreath, RGB lamp, candles, and witchy extras. Size is one-size / listed on the dress. Swap add-ons at the same $149 price.", [
    ...
    hamper("movie-night-boo-box-hamper", "Movie-Night Boo Box Hamper", 179, "Witch comforter, hoodie, lamp, socks, and cozy October extras.", "Curl-up October in a box: 8-piece witch comforter, hoodie, plush pillow, socks, RGB lamp, and candles. The hamper price stays $179 when you swap included items for add-ons.", [
    ...
    hamper("porch-yard-takeover-hamper", "Porch & Yard Takeover Hamper", 199, "Two inflatables, tombstone, path lights, and outdoor ghosts.", "A street-visible yard kit: witch inflatable, yard inflatable, tombstone, pumpkin inflatable, garden house, and path lights. Free shipping. Swap add-ons at the same $199 price.", [
    ...
    hamper("princess-bedroom-party-hamper", "Princess Bedroom Party Hamper", 229, "Two princess looks, pumpkin bedding, lamp, and party extras.", "Grandparent-ready gift: girls costume set, Aurora dress, cloak, pumpkin comforter, RGB lamp, lantern, and party extras. Swap included items at the same $229 price.", [
    ...
    hamper("indoor-haunted-house-hamper", "Indoor Haunted House Hamper", 249, "Voice skull, hanging ghosts, tombstone, inflatable, and backdrop.", "Turn a room into a haunted house: voice-control skull, witch inflatable, hanging ghosts, tombstone, RGB lamp, and horror backdrop. Customize with add-ons — $249 stays the hamper price.", [
        line("halloween-witch-inflation-model-indoor-and-outdoor-decoration-25092301", "Witch inflatable", 63.84, YARD),
    ...
    hamper("whole-house-halloween-hq-hamper", "Whole-House Halloween HQ Hamper", 299, "Yard inflatable, witch inflatable, bedding, costume, and party lighting.", "One decision for the whole house: comforter, witch inflatable, adult costume dress, yard inflatable, RGB lamp, balloons, lantern, and socks. $299 hamper price does not change when you swap included items.", [
...
exports.HALLOWEEN_HAMPERS_CATEGORY = {
    name: "Halloween Hampers",
    slug: constants_1.HALLOWEEN_HAMPERS_CATEGORY_SLUG,
    description: "Ready-to-gift Halloween hampers — curated décor, costume, and party kits with free shipping over $49. Swap included items for add-ons at the same hamper price.",
    sortOrder: 0,
};
function isHalloweenHamperProduct(product) {
    if (product.categorySlug === constants_1.HALLOWEEN_HAMPERS_CATEGORY_SLUG)
        return true;
    if (product.tags?.includes(exports.HAMPER_TAG))
        return true;
    if (product.slug && exports.HALLOWEEN_HAMPER_DEFS.some((h) => h.slug === product.slug))
        return true;
    return false;
}
function isStorefrontVisibleProduct(product) {
    if (isHalloweenHamperProduct(product))
        return true;
    return product.vendorSlug === "cj-dropshipping" || Boolean(product.cjPid);
}
function hamperContentsValue(contents) {
    return contents.reduce((sum, item) => sum + item.price, 0);
}
function addonsForHamper(contents) {
    const used = new Set(contents.map((c) => c.slug));
    return exports.HAMPER_ADDON_POOL.filter((a) => !used.has(a.slug));
}
function hamperCustomizationSignature(custom) {
    if (!custom)
        return "";
...
function cartLinesMatch(a, b) {
    ...
    if (aHamper || bHamper)
        return aHamper === bHamper;
    return (0, product_addons_1.cartAddonSignature)(a.addons) === (0, product_addons_1.cartAddonSignature)(b.addons);
}
function resolveHamperCustomization(product, custom) {
    ...
    for (const slug of excluded) {
        if (!contentSlugs.has(slug))
            return { ok: false, error: `Cannot exclude ${slug} — it is not in this hamper` };
    }
    ...
    for (const slug of extrasIn) {
        ...
        if (!def)
            return { ok: false, error: "Extra add-on must be from this hamper's add-on list" };
        extraAddons.push({
...
function buildHalloweenHamperCatalogProducts() {
    return exports.HALLOWEEN_HAMPER_DEFS.map((def) => {
        ...
        return {
            ...
            vendorSlug: constants_1.VENDOR_HALLOWEENREADY,
            published: true,
            couponExcluded: true,
...
function getHalloweenHamperDef(slug) {
    return exports.HALLOWEEN_HAMPER_DEFS.find((h) => h.slug === slug);
}
