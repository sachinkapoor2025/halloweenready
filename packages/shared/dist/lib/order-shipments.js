...
exports.addressFingerprint = addressFingerprint;
exports.validateShipmentsPartitionCart = validateShipmentsPartitionCart;
exports.buildOrderShipments = buildOrderShipments;
exports.singleCheckoutShipment = singleCheckoutShipment;
const currency_1 = require("../currency");
const free_shipping_1 = require("./free-shipping");
...
function validateShipmentsPartitionCart(cartItems, shipments) {
    if (!shipments.length)
...
function takeItemsFromCart(cartBySlug, lines) {
    const out = [];
    for (const line of lines) {
        const source = cartBySlug.get(line.productSlug);
        if (!source)
            return `Unknown product: ${line.productSlug}`;
        out.push({ ...source, quantity: line.quantity });
    }
    return out;
}
function buildOrderShipments(input) {
    const partitionError = validateShipmentsPartitionCart(input.cartItems, input.checkoutShipments);
    if (partitionError)
    ...
    for (let i = 0; i < input.checkoutShipments.length; i++) {
        ...
        built.push({
            ...
            items,
            subtotal: (0, currency_1.cartSubtotal)(items),
        });
    }
    ...
    const perAddress = built.map((s) => (0, free_shipping_1.quoteAddressShipmentShipping)({
        items: s.items,
...
function singleCheckoutShipment(shippingAddress, cartItems) {
    return {
        shippingAddress,
