...
exports.DEFAULT_MARKETS = exports.DEFAULT_VENDORS = exports.DEFAULT_WAREHOUSES = exports.WAREHOUSE_IN_ID = exports.WAREHOUSE_UK_ID = exports.WAREHOUSE_US_ID = exports.DEFAULT_MARKET_COUNTRY = exports.EUROPEAN_COUNTRY_CODES = void 0;
exports.normalizePhoneDigits = normalizePhoneDigits;
exports.validatePostalCode = validatePostalCode;
exports.warehouseServesCountry = warehouseServesCountry;
exports.warehouseServesPostal = warehouseServesPostal;
exports.eligibleWarehouses = eligibleWarehouses;
exports.estimateDeliveryDays = estimateDeliveryDays;
exports.assignFulfillment = assignFulfillment;
exports.productAvailableInCountry = productAvailableInCountry;
exports.publicMarketContact = publicMarketContact;
exports.marketByCountry = marketByCountry;
exports.marketBySlug = marketBySlug;
exports.checkoutCurrencyForMarket = checkoutCurrencyForMarket;
const constants_1 = require("../constants");
/** European country codes we may serve from the UK warehouse (never a fake "EU" country). */
exports.EUROPEAN_COUNTRY_CODES = [
    "GB",
    "IE",
    "DE",
    "FR",
...
exports.DEFAULT_MARKET_COUNTRY = "US";
exports.WAREHOUSE_US_ID = "us-warehouse";
exports.WAREHOUSE_UK_ID = "uk-warehouse";
exports.WAREHOUSE_IN_ID = "in-warehouse";
function normalizePhoneDigits(phone) {
    return (phone ?? "").replace(/\D/g, "");
}
exports.DEFAULT_WAREHOUSES = [
    {
        warehouseId: exports.WAREHOUSE_US_ID,
        warehouseCode: "US-SJ",
        name: "US Warehouse",
    ...
    {
        warehouseId: exports.WAREHOUSE_UK_ID,
        ...
        priority: 20,
        serviceArea: {
            countryCodes: [...exports.EUROPEAN_COUNTRY_CODES],
            stateOrRegionCodes: [],
            postalPrefixes: [],
    ...
    {
        warehouseId: exports.WAREHOUSE_IN_ID,
...
exports.DEFAULT_VENDORS = [
    {
        vendorId: constants_1.VENDOR_HALLOWEENREADY,
        slug: constants_1.VENDOR_HALLOWEENREADY,
        name: "OccasionFun",
        countryCode: "US",
        companyOwned: true,
        active: true,
        warehouseIds: [exports.WAREHOUSE_US_ID, exports.WAREHOUSE_UK_ID, exports.WAREHOUSE_IN_ID],
        userEmails: [],
        users: [],
        contactEmail: "support@occasionfun.com",
        contactPhone: "+1 (669) 236-1526",
        priority: 10,
    },
    {
        vendorId: constants_1.VENDOR_ORANGE_COUNTY,
        slug: constants_1.VENDOR_ORANGE_COUNTY,
        name: "Orange County",
        countryCode: "US",
        companyOwned: false,
        active: true,
        warehouseIds: [exports.WAREHOUSE_US_ID],
        userEmails: [],
        users: [],
        contactEmail: "",
        contactPhone: "",
...
function contactFromWarehouse(wh, extras = {}) {
    return {
        phone: wh.phone,
...
const usWh = exports.DEFAULT_WAREHOUSES[0];
const ukWh = exports.DEFAULT_WAREHOUSES[1];
const inWh = exports.DEFAULT_WAREHOUSES[2];
exports.DEFAULT_MARKETS = [
    {
        countryCode: "US",
        name: "United States",
        slug: "us",
        ...
        defaultWarehouseId: exports.WAREHOUSE_US_ID,
    ...
    },
    {
        ...
        defaultWarehouseId: exports.WAREHOUSE_UK_ID,
...
function validatePostalCode(countryCode, postalCode) {
    ...
    if (exports.EUROPEAN_COUNTRY_CODES.includes(country)) {
...
function warehouseServesCountry(warehouse, countryCode) {
    if (!warehouse.active || !warehouse.fulfillmentEnabled)
        return false;
    const country = countryCode.trim().toUpperCase();
    const area = warehouse.serviceArea;
    if (area.countryCodes.includes(country))
        return true;
    if (area.internationalShipping && warehouse.countryCode !== country)
        return true;
    return false;
}
function warehouseServesPostal(warehouse, countryCode, postalCode) {
    if (!warehouseServesCountry(warehouse, countryCode))
        return false;
...
function eligibleWarehouses(opts) {
    const { warehouses, countryCode, postalCode, vendorId } = opts;
    return warehouses
        .filter((w) => {
        if (vendorId && w.vendorId && w.vendorId !== vendorId)
            return false;
...
function vendorForItem(vendors, vendorSlug) {
    const slug = (vendorSlug ?? "").trim() || constants_1.VENDOR_HALLOWEENREADY;
    return vendors.find((v) => v.slug === slug || v.vendorId === slug);
}
function estimateDeliveryDays(warehouse, countryCode) {
    const country = countryCode.trim().toUpperCase();
    if (warehouse.countryCode === country)
        return warehouse.countryCode === "US" ? 4 : 3;
    if (exports.EUROPEAN_COUNTRY_CODES.includes(country)) {
        return warehouse.countryCode === "GB" ? 5 : 8;
    }
    if (country === "CA")
        return 6;
    if (country === "AU" || country === "AE")
        return 10;
    return 12;
}
function assignFulfillment(opts) {
    const country = opts.destinationCountry.trim().toUpperCase() || exports.DEFAULT_MARKET_COUNTRY;
    const splitsByKey = new Map();
    for (const item of opts.items) {
        const vendor = vendorForItem(opts.vendors, item.vendorSlug);
        const vendorId = vendor?.vendorId ?? constants_1.VENDOR_HALLOWEENREADY;
        const listing = opts.listings?.find((l) => l.active &&
        ...
        const vendorWarehouses = opts.warehouses.filter((w) => {
            if (listing && listing.warehouseId === w.warehouseId)
                return true;
            if (!vendor)
                return true;
            if (!vendor.warehouseIds.length)
                return !w.vendorId || w.vendorId === vendorId;
        ...
        const warehouse = eligible[0] ?? opts.warehouses.find((w) => w.warehouseId === exports.WAREHOUSE_US_ID);
        if (!warehouse)
            continue;
...
function productAvailableInCountry(opts) {
    ...
    if (listed.length)
        return listed.includes(country);
...
function publicMarketContact(market) {
    return { ...market.contact, countryCode: market.contact.countryCode ?? market.countryCode };
}
function marketByCountry(markets, countryCode) {
    const code = countryCode.trim().toUpperCase();
    return markets.find((m) => m.countryCode === code);
}
function marketBySlug(markets, slug) {
    const s = slug.trim().toLowerCase();
    return markets.find((m) => m.slug === s);
}
function checkoutCurrencyForMarket(market) {
    if (market?.checkoutCurrency === "INR")
        return "INR";
    return "USD";
}
