 ...
 *
 * Usage (against AWS):
 *   ENVIRONMENT=prod LEGACY_TABLE=halloweenready-prod npx tsx scripts/migrate-to-multitable.ts
 *
 * Safe to re-run (idempotent puts). The legacy table is only read, never deleted.
 */
import { DynamoDBClient } from "@aws-sdk/client-dynamodb";
import { DynamoDBDocumentClient, ScanCommand, PutCommand } from "@aws-sdk/lib-dynamodb";
import { orderKeys, customerKeys, ORDER_STATUS } from "@halloweenready/shared";

const ENV = process.env.ENVIRONMENT ?? "dev";
const LEGACY_TABLE = process.env.LEGACY_TABLE ?? `halloweenready-${ENV}`;
const ORDERS_TABLE = process.env.ORDERS_TABLE ?? `halloweenready-orders-${ENV}`;
const CUSTOMERS_TABLE = process.env.CUSTOMERS_TABLE ?? `halloweenready-customers-${ENV}`;
