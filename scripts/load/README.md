# Load testing — HalloweenReady (~1000 concurrent shoppers)
...
| Staging | SAM stack `halloweenready-staging` | Prefer this for 1000 VU runs |
...
- Lambda `halloweenready-api-staging` (or `-prod`): `ConcurrentExecutions`, `Duration`, `Errors`, `Throttles`
- DynamoDB tables `halloweenready-*-staging`: `ConsumedReadCapacityUnits`, `ConsumedWriteCapacityUnits`, `ThrottledRequests`
...
aws cloudwatch get-metric-statistics --namespace AWS/Lambda \
  --metric-name ConcurrentExecutions --dimensions Name=FunctionName,Value=halloweenready-api-staging \
