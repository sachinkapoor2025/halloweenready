...
const TABLES: {
  ...
  { name: process.env.PRODUCTS_TABLE ?? `halloweenready-products-${ENV}`, indexes: ["GSI1"] },
  { name: process.env.ORDERS_TABLE ?? `halloweenready-orders-${ENV}`, indexes: ["GSI1", "GSI2", "GSI3"] },
  { name: process.env.CARTS_TABLE ?? `halloweenready-carts-${ENV}`, indexes: ["GSI1"] },
  { name: process.env.CUSTOMERS_TABLE ?? `halloweenready-customers-${ENV}`, indexes: ["GSI1"] },
  { name: process.env.EVENTS_TABLE ?? `halloweenready-events-${ENV}`, indexes: ["GSI1"] },
  { name: process.env.CONFIG_TABLE ?? `halloweenready-config-${ENV}`, indexes: [] },
];
