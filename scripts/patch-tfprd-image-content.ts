...
function buildDescription(name: string, spec: Spec): string {
  ...
  return [
    `<p><strong>${esc(name)}</strong> is a premium designer Rakhi choice for USA delivery. Sisters in India, the UK, Canada, Australia, and worldwide order from HalloweenReady so brothers across America receive festive packaging with reliable domestic shipping for Raksha Bandhan.</p>`,
    ...
    `<p><strong>Why sisters choose HalloweenReady for USA delivery:</strong></p>`,
...
function buildSeoDescription(name: string, spec: Spec): string {
  ...
  return metaDescription(
    `Send ${name} to USA — ${qty}${ritual}. Domestic shipping for Raksha Bandhan from HalloweenReady.`
  );
}
...
async function main() {
  ...
  const TABLE = process.env.PRODUCTS_TABLE ?? `halloweenready-products-${ENV}`;
