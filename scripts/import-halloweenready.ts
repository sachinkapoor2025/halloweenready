/**
 * Import categories + products from halloweenready.com (WooCommerce Store API)
 * into DynamoDB. Also writes scripts/data/halloweenready-catalog.json for local seed.
 *
 * Usage:
 *   npm run import:halloweenready -- --fetch-only           # refresh catalog JSON
...
async function fetchGalleryImages(slug: string): Promise<string[]> {
  try {
    const res = await fetch(`https://halloweenready.com/product/${encodeURIComponent(slug)}/`, {
      headers: { "User-Agent": "HalloweenReady-Catalog-Import/1.0" },
    });
    if (!res.ok) return [];
...
async function enrichProductImages(products: CatalogProduct[]): Promise<void> {
  ...
  async function worker() {
    while (index < products.length) {
      ...
      if (merged.length > 0) product.images = merged;
...
async function main() {
  ...
  if (!refresh && !fetchOnly && existsSync(CATALOG_PATH)) {
