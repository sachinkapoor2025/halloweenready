 ...
 *   MARKETING_SMTP_USER  (default order@halloweenready.com)
...
const TABLE = process.env.EMAIL_CAMPAIGNS_TABLE ?? `halloweenready-email-campaigns-${ENV}`;
...
async function main() {
  ...
  const merged = {
    ...
    smtpSecure,
    smtpUser:
      process.env.MARKETING_SMTP_USER?.trim() || prev.smtpUser || "order@halloweenready.com",
    smtpPassword: pass,
  };
