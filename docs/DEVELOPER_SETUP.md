...
and `CONFIG_TABLE` (each defaulting to `halloweenready-<domain>-<ENVIRONMENT>`). Create the
local tables with `npm run db:setup` (or set `USE_MEMORY_DB=true`).

Migrating from the legacy single table? Run:

```bash
ENVIRONMENT=prod LEGACY_TABLE=halloweenready-prod npm run migrate:multitable
```
